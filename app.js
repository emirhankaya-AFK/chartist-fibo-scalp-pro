  ...
  try {
    ...
    if (trackingList) {
      trackingList.innerHTML = tracking.length ? tracking.map((item) => {
        ...
        const analystMeta = item.strategy === "Analist Alarmı" ? `<p><b>Alarm fiyatı:</b> ${formatCurrency(item.alarmPrice)} · <b>En yüksek:</b> ${formatCurrency(item.highestPrice)} <span class="positive">(maks. ${item.maxChangePercent >= 0 ? "+" : ""}${formatNumber(item.maxChangePercent)}%)</span></p><small>${escapeHtml(item.analystSource || "Analist")} · ${escapeHtml(item.alarmType || "Seviye alarmı")}</small>` : "";
        return `<article class="tracking-item"><div class="notification-item-head"><strong>${escapeHtml(item.ticker)} · ${item.strategy === "Analist Alarmı" ? "analist performansı" : "takip"}</strong><time>${startAt} → ${lastAt}</time></div>${item.analystMessage ? `<p class="tracking-note">${escapeHtml(item.analystMessage)}</p>` : ""}<p><b>${formatCurrency(item.startPrice)}</b> başlangıç → <b>${formatCurrency(item.lastPrice)}</b> son fiyat <span class="${change >= 0 ? "positive" : "negative"}">(${change >= 0 ? "+" : ""}${formatNumber(change)}%)</span></p>${analystMeta}<small>${item.updates?.length || 1} fiyat gözlemi · ${escapeHtml(item.strategy || "")}</small></article>`;
